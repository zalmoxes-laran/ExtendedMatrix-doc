...
**Allowed Connections:**
  * Source: Special Find (SF), Virtual Special Find (VSF), Reused Special Find (RSF), Stratigraphic Unit (US), Documentary Stratigraphic Unit (USD), Virtual Stratigraphic Unit (USV), Functional Unit
...
.. note:: **CIDOC CRM projection.**

   Every containment is exported as ``crm:P46i_forms_part_of``, the generic mereological
   property. When the container is a **US**, a second and more precise triple goes out
   beside it: ``crmarchaeo:AP21i_is_contained_in``, the CRMarchaeo shortcut for the path
   *E18 Physical Thing → A7 Embedding → A2 Stratigraphic Volume Unit*. This is the
   archaeologically correct statement for a find inside a deposit — the object is
   contained in the unit, not a constituent part of it — and it is a refinement rather
   than a correction, since CRMarchaeo declares ``AP18`` a subproperty of ``P46``.

   The refinement is withheld for the other containers on purpose: ``AP21i`` ranges
   strictly over ``A2 Stratigraphic Volume Unit``, and only the US maps to A2 (a USD and
   the virtual units map to ``A8``, a VSF to ``E89``/``E19``). Saying *how* an object sits
   in a deposit — embedded since the deposit formed, or intrusive — needs the full
   ``A7 Embedding`` node, which is not modelled and is an open point with CRMarchaeo.
